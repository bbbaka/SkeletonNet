    ...
    def __init__(self, rootimg = ROOT_IMG, rootpc = ROOT_PC, class_choice = "chair",
        ...
        for item in self.cat:
            if self.SVR:
                ...
                fns1 = open(os.path.join(ROOT_SPLIT, self.cat[item]+'_'+TRAIN_SPLIT_FILENAME),'r').readlines()
                fns1 = [fn.strip() for fn in fns1]
                fns2 = open(os.path.join(ROOT_SPLIT, self.cat[item]+'_'+TEST_SPLIT_FILENAME),'r').readlines()
                fns2 = [fn.strip() for fn in fns2]
                fns = fns1 + fns2
